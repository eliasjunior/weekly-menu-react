...
import mockData from "./tests/shopping.list.mock.data";
...
describe("ShoppingListUtilService", () => {
  it("should add attrs to product", () => {
    ...
    const recipesChanged = ShoppingListUtilService.addRecInfoToProduct(
      attrs.recipes
    );
  ...
  it("should merge recipe and categories case 1", () => {
    ...
    const recipes = ShoppingListUtilService.addRecInfoToProduct(
      shoppinpListData.recipes
    );

    const mergeCategories = ShoppingListUtilService.mergeCategories(
      recipes,
      categories
    );
  ...
  it("should check prod recipes include", () => {
    ...
    const result = ShoppingListUtilService.updateRecipesSelection(
      recipes,
      itemSelected
    );
